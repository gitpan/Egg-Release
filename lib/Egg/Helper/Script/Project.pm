...
sub rcparam {
	my($self)= @_;
	my $rc= Egg::Plugin::YAML::Tool->loadrc || {};
...
sub generate {
	...
	my $rc= $self->rcparam;
	my $current= getcwd();
	$self->create_dir($self->{base})
	 || die qq/$self->{base} directory cannot be made./;
	chdir($self->{base})
	 || die q/The directory cannot be moved./;
	eval {
		...
		{
			my @base= split /[\\\/]+/, $rc->{base};
			$rc->{"$_\_dir"}= File::Spec->catfile(@base, $_)
			  for qw/root cache tmp comp htdocs/;
		  };
		$self->output_files($rc, join '', <DATA>);
		$self->create_dir("$self->{base}/$_") for qw/root cache tmp comp/;
		if (File::Spec->isa('File::Spec::Unix')) {
			...
			$self->output_file( $rc, {
			  filename=> 'MANIFEST',
			  value=> $self->manifest_default($rc),
			  } );
		}
	  };
	chdir($current);
	if (my $err= $@) {
		$self->remove_dir($self->{base});
		print <<ERROR;
...
* Please make a wonderful project, good luck !!

= Please make package as follows.
...
sub output_files {
	my $self= shift;
	my $attr= shift || die q/I want parameter./;
	my $data= shift || die q/I want yaml data./;
	$self->output_file($self->setup_dist_name($attr, $_), $_)
	  for ( YAML::Load($data) );
}
sub setup_dist_name {
	my($self, $attr, $data)= @_;
	...
	$name=~s{^/+} [];
	$name=~s{\.[^\.]+$} [];
	return $attr if (! $name || $name=~m{/$});
	$attr->{dist}= join '::', (split /\/+/, $name);
	return $attr;
}
sub manifest_default {
	my($self, $rc)= @_;
	<<MANIFEST_FILE;
Build.PL
Changes
...
lib/$rc->{project}.pm
lib/$rc->{project}/D.pm
lib/$rc->{project}/D/Root.pm
lib/$rc->{project}/config.pm
t/01_$rc->{project}.t
t/02_$rc->{project}-config.t
t/03_$rc->{project}-D.t
t/04_$rc->{project}-D-Root.t
t/89_pod.t
MANIFEST_FILE
}
...
---
filename: bin/prototype_generator.pl
permission: 0755
value: |
  #!<# perl_path #>
  use lib qw(<# base #>/lib);
...
---
filename: bin/yaml_generator.pl
permission: 0755
value: |
  #!<# perl_path #>
  use lib qw(<# base #>/lib);
...
---
filename: bin/create_dispatch.pl
permission: 0755
value: |
  #!<# perl_path #>
  use lib qw(<# base #>/lib);
...
---
filename: etc/egg_mod_perl.conf
value: |
  LoadModule perl_module modules/mod_perl.so
  # PerlSwitches  -w
  PerlSwitches -I<# base #>/lib
  PerlRequire  <# base #>/bin/egg_startup.pl
...
value: |
  ...
  <# document #>
---
filename: lib/<# project #>/D/Root.pm
filetype: module
value: |
  package <# project #>::D::Root;
  #
  # Copyright (C) <# headcopy #>, All Rights Reserved.
  # <# author #>
  #
...
value: |
  use Test::More tests => 1;
  BEGIN { use_ok('<# project #>::D') };
---
filename: t/04_<# project #>-D-Root.t
value: |
  use Test::More tests => 1;
  BEGIN { use_ok('<# project #>::D::Root') };
---
filename: t/89_pod.t
value: |
  use Test::More;
...
value: |
  ...
  $self->{version}  <# gmt #>
  	- original version; created by Egg v<# egg_version #> with
  	  module name <# project #>.
---
filename: README
value: |
